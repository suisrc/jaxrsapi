...
public class ClientServiceFactory {
     ...
     * 单接口多服务器模式
     */
    static boolean MULIT_MODE = Boolean.valueOf(System.getProperty(Consts.KEY_REMOTE_API_NULTI_MODE, "false"));
    ...
    private void createMethodInfo(ApiActivator activator, Index index, JJClass jjc, MethodInfo method)
        ...
        // 分类
        List<AnnotationInstance> threadValue_ms = new ArrayList<>();
        ...
        for (AnnotationInstance anno : annos_m) {
            ...
            if (annoName.equals(ThreadValue.class.getName())) {
                threadValue_ms.add(anno);
            } else if (annoName.equals(Value.class.getName())) {
                value_ms.add(anno);
        ...
        // -----------------------------------------------------------------------------------ZERO parameter ThreadValue参数获取部分
        for (AnnotationInstance anno : threadValue_ms) { // ThreadValue
            JCall methodExpr = getThreadValueMethodExpr(jjm, anno);
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            List<AnnotationInstance> annos_f = classInfo.annotations().get(DotName.createSimple(ThreadValue.class.getName()));
            ...
            // -----------------------------------------------------------------------------------ZERO field Value参数获取部分
            annos_f = classInfo.annotations().get(DotName.createSimple(Value.class.getName()));
            if (annos_f != null && !annos_f.isEmpty()) {
            ...
            if (annos_f != null && !annos_f.isEmpty()) {
                for (AnnotationInstance anno : annos_f) { // DefaultValue
                    ...
                    for ( AnnotationInstance ai : anno.target().asField().annotations()) {
                        if (ai.name().toString().equals(TfDefaultValue.class.getName())) {
                            tfAnno = ai;
                            break;
                        }
                    }
                    JCall methodExpr = getDefaultValueMethodExpr(jjm, anno, tfAnno, parameters.get(i));
                    createFieldValueInfo(jjm, body, params.get(i), anno, methodExpr);
                }
            }
            // -------------------------------------------------------------------------------ZERO 最后的数据修正拦截
     ...
     * @return
     */
    private JCall getThreadValueMethodExpr(JJMethod jjm, AnnotationInstance anno) {
        // ThreadValue.class
        ...
        String value = anno.value().asString();
        // 静态赋值方法
        jjm.getJJClass()._import(actname);
        JCall methodExpr = JExprs.callStatic(actname, metname);
        methodExpr.arg(JExprs.str(value));
        return methodExpr;
    }
    ...
    private void creatReturnInfo(JJMethod jjm, JBlock body, MethodInfo method, List<JParamDeclaration> params) {
        ...
        if (anno != null) {
            ...
            // methodExpr.arg(baseUrl.plus(JExprs.str(getMethodUri(method))));
    ...
    private void createClassInfo(ApiActivator activator, JJClass jjc, ClassInfo classInfo)
        ...
        if (MULIT_MODE) { // 单接口多服务器模式
            ...
            jjc.annotateValue(anno, "value", named.value() + Consts.separator + jjc.getCanonicalName());
        }
        // 继承的接口
        jjc._import(api);
    ...
    public static void processIndex(Index index, BiConsumer<Class<?>, CtClass> acceptThen) throws Exception {
        ...
        for (Class<?> classActivator : activatorSet) {
            ...
            ClientServiceFactory.createImpl(activator, index, acceptThen);
        }
    }
    ...
    static void createImpl(ApiActivator activator, Index index, String targetFile) throws Exception {
        ...
        createImpl(activator, index, jjst, jj -> jj.writeSource4File());
    }
