...
public @interface NonProxy {
    ...
    /**
     * 在实际编程的时候，很多时候，禁用是后期更改的，但是会遇到无法修改代码的问题 所以在这里，
     * 我们通过参数，获取该Proxy是否有效，当然这个就是需要远程访问 代码的运行时构建的特性。
     * 
     * 远程访问代码生成的时候，会通过该字段的value作为key想系统变量中查找对应的值 如果值为true，表示禁用该远程接口。
     * 默认情况下，如果value为""表示该接口永久禁用
